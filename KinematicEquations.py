...
class KinematicEquations:
    ...
    def __init__(self, velocityFinal: float, velocityInitial: float, time: float, deltaDistance: float, acceleration: float):
        ...
        if sum(unknowns) > 2:
            ...
            if (unknowns[GC.VF]) and not (unknowns[GC.VI] or unknowns[GC.T] or unknowns[GC.DD] or unknowns[GC.A]):
                if GC.DEBUG_STATEMENTS_ON: print(f"Using {self.eq4} equation since ONLY final velocity = {velocityFinal} is unknown")
                ...
                if GC.DEBUG_STATEMENTS_ON: print(f"Using {self.eq2} equation since final velocity = {velocityFinal} & time = {time} are unknown")
                ...
                if GC.DEBUG_STATEMENTS_ON: print(f"Using {self.eq3} equation since final velocity = {velocityFinal} & delta distance = {deltaDistance} are unknown")
                ...
                if GC.DEBUG_STATEMENTS_ON: print(f"Using {self.eq5} equation since final velocity = {self.vf} & acceleration = {a} are unknown")
            ...
            # Calculate Time
                ...
                if GC.DEBUG_STATEMENTS_ON: print(f"Using {self.eq1} equation since ONLY time = {t} is unknown and acceleration does NOT equal 0")
    ...
    def unit_test():
        ...
        if GC.DEBUG_STATEMENTS_ON: f"Initial Velocity: {scalarInput} m/s = [{xVelocity}, {yVelocity}, {zVelocity}]"
        ...
        if GC.DEBUG_STATEMENTS_ON: f"Final Velocity: {scalarOutput} m/s = [{xAxis.vf}, {yAxis.vf}, {zAxis.vf}]"
